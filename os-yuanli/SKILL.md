...
### Protocols（Tier 1 · 能力包 · v0.2+）

可按需加载，kernel 仍可独立运行。

- [protocols/task-protocol.md](protocols/task-protocol.md): 任务文件契约、frontmatter、状态机
- [protocols/router-five-axes.md](protocols/router-five-axes.md): 路由五维排序与硬门槛
- [protocols/brain-tiers.md](protocols/brain-tiers.md): Strategy / Execute / Knowledge 三 tier 与成本约束
- [protocols/guides-and-sensors.md](protocols/guides-and-sensors.md): 前馈 / 反馈双控回路
- [protocols/evolution-flywheel.md](protocols/evolution-flywheel.md): 进化三通道与反模式
- [protocols/truth-hierarchy.md](protocols/truth-hierarchy.md): 五层真源与冲突裁决
- [protocols/backflow-channels.md](protocols/backflow-channels.md): 本地 / 异步 / 外部 三通道契约
- [protocols/layer-dependencies.md](protocols/layer-dependencies.md): L0–L4 分层与依赖硬规则

### Templates（Tier 2 · 适配脚手架 · v0.3+）

同事按自己的主题 / runtime / 外部系统实例化。复制 `templates/` 到工作目录后填写即可。

- [templates/themes.yaml.template](templates/themes.yaml.template): 主题枚举（对应 task-protocol 的 Exclusion）
- [templates/runtime.yaml.template](templates/runtime.yaml.template): 执行环境声明（对应 brain-tiers 的 Exclusion）
- [templates/bridges.yaml.template](templates/bridges.yaml.template): 回流与外部桥（对应 backflow-channels 的 Exclusion）
- [templates/cockpit.template.md](templates/cockpit.template.md): 指挥中心 · 3 种变体
- [templates/task.template.md](templates/task.template.md): 合规任务 .md 模板
- [templates/daemon.template.yaml](templates/daemon.template.yaml): 定时巡检（可选，对应 guides-and-sensors 的 Sensors）
